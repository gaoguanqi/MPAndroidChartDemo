package com.maple.smaple.mpandroidchartdemo.utils;
...
import com.maple.smaple.mpandroidchartdemo.interfaces.OnUpdateListener;

/**
 * Created by gaogu on 2018/1/9.
 */
...
//                textView.setTextColor((Integer) animation.getAnimatedValue());
    ...
    public static void cardFilpAnimation(final View beforeView, final View AfterView) {
        ...
        if (beforeView.getVisibility() == View.GONE) {
            ...
            RxImageTool.invisToVis = ObjectAnimator.ofFloat(beforeView,
                    "rotationY", -90f, 0f);
            RxImageTool.visToInvis = ObjectAnimator.ofFloat(AfterView,
                    "rotationY", 0f, 90f);
        } else if (AfterView.getVisibility() == View.GONE) {
            RxImageTool.invisToVis = ObjectAnimator.ofFloat(AfterView,
                    "rotationY", -90f, 0f);
            RxImageTool.visToInvis = ObjectAnimator.ofFloat(beforeView,
                    "rotationY", 0f, 90f);
        }

        RxImageTool.visToInvis.setDuration(250);// 翻转速度
        RxImageTool.visToInvis.setInterpolator(accelerator);// 在动画开始的地方速率改变比较慢，然后开始加速
        RxImageTool.invisToVis.setDuration(250);
        RxImageTool.invisToVis.setInterpolator(decelerator);
        RxImageTool.visToInvis.addListener(new Animator.AnimatorListener() {

            @Override
            public void onAnimationEnd(Animator arg0) {
                if (beforeView.getVisibility() == View.GONE) {
                    AfterView.setVisibility(View.GONE);
                    RxImageTool.invisToVis.start();
                    beforeView.setVisibility(View.VISIBLE);
                } else {
                    AfterView.setVisibility(View.GONE);
                    RxImageTool.visToInvis.start();
                    beforeView.setVisibility(View.VISIBLE);
                }
            }
